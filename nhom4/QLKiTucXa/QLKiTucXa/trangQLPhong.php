...
<div class="main-content">
    <center><h3 class="mt-2">Quản Lý Phòng</h3></center>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php
            include 'conn.php';
            $sql = "SELECT Phong.*, KhuVuc.TenKhuVuc, LoaiPhong.TenLoaiPhong FROM Phong INNER JOIN KhuVuc ON Phong.KhuVucID = KhuVuc.KhuVucID INNER JOIN LoaiPhong ON Phong.LoaiPhongID = LoaiPhong.LoaiPhongID";
            ...
            if (count($result) > 0) {
                foreach ($result as $row) {
                    ...
                    echo "<button type='button' class='btn btn-light mr-1' onclick='deletePhong({$row['PhongID']})'>";
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addFormPhong" action="themPhong.php" method="post">
                    
                    <div class="input-group mb-3">
                        <span class="input-group-text col-2">Tên phòng</span>
                        <input type="text" class="form-control" id="addTenPhong" name="TenPhong" />
                    </div>
                    <div class="input-group mb-3">
                        <label for="addKhuVucID" class="input-group-text col-2">Khu vực</label>
                        <select class="form-select col-10" id="addKhuVucID" name="KhuVucID">
                            <?php
                            ...
                            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($result as $row) {
                                echo "<option value='{$row['KhuVucID']}'>{$row['TenKhuVuc']}</option>";
                            }
                            ?>
                        </select>
                    </div>
                    <div class="input-group mb-3">
                        <label for="addLoaiPhongID" class="input-group-text col-2">Loại phòng</label>
                        <select class="form-select col-10" id="addLoaiPhongID" name="LoaiPhongID">
                            <?php
                            ...
                            $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($result as $row) {
                                echo "<option value='{$row['LoaiPhongID']}'>{$row['TenLoaiPhong']}</option>";
                            }
                            ?>
                        </select>
                    </div>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editFormPhong" action="suaPhong.php" method="post">
                    <div class="input-group mb-3">
                        <span class="input-group-text col-2">ID</span>
                        <input type="text" class="form-control" id="editPhongID" name="PhongID" readonly />
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text col-2">Tên phòng</span>
                        <input type="text" class="form-control" id="editTenPhong" name="TenPhong" />
                    </div>
                    <div class="input-group mb-3">
                        <label for="editKhuVucID" class="input-group-text col-2">Khu vực</label>
                        <select class="form-select col-10" id="editKhuVucID" name="KhuVucID">
                            <?php
                            $sql = "SELECT * FROM KhuVuc";
                            $stmt = $conn->prepare($sql);
                            $stmt->execute();
                    ...
                    <div class="input-group mb-3">
                        <label for="editLoaiPhongID" class="input-group-text col-2">Loại phòng</label>
                        <select class="form-select col-10" id="editLoaiPhongID" name="LoaiPhongID">
                            <?php
                            $sql = "SELECT * FROM LoaiPhong";
                            $stmt = $conn->prepare($sql);
                            $stmt->execute();
...
<script>
function editClick(phong) {
    document.getElementById('editPhongID').value = phong.PhongID;
    document.getElementById('editTenPhong').value = phong.TenPhong;
    document.getElementById('editKhuVucID').value = phong.KhuVucID;
    document.getElementById('editLoaiPhongID').value = phong.LoaiPhongID;
}
